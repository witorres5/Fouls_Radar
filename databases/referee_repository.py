...
class RefereeRepository:
    ...
    def get_referees_summary_by_league(self, league_id: int, season: int) -> List[Dict[str, Any]]:
        """Consulta métricas de árbitros calculadas directamente sobre match_fixtures."""
        with self.db_manager.get_connection() as conn:
            ...
            query_fixtures = """
                SELECT 
                    ...
                    SUM(CASE WHEN UPPER(COALESCE(status, '')) IN ('FT', 'MATCH FINISHED', 'AET', 'PEN') THEN 1 ELSE 0 END) as finished_matches,
                    SUM(CASE WHEN UPPER(COALESCE(status, '')) IN ('FT', 'MATCH FINISHED', 'AET', 'PEN') THEN COALESCE(total_fouls, 0) ELSE 0 END) as total_fouls,
                    ROUND(
                        CAST(SUM(CASE WHEN UPPER(COALESCE(status, '')) IN ('FT', 'MATCH FINISHED', 'AET', 'PEN') THEN COALESCE(total_fouls, 0) ELSE 0 END) AS FLOAT) 
                        / NULLIF(SUM(CASE WHEN UPPER(COALESCE(status, '')) IN ('FT', 'MATCH FINISHED', 'AET', 'PEN') THEN 1 ELSE 0 END), 0), 
                        2
                ...
                WHERE CAST(league_id AS INTEGER) = CAST(? AS INTEGER)
                  ...
                  AND LOWER(TRIM(referee_name)) NOT IN ('sin árbitro registrado', 'árbitro no asignado', 'n/a', 'null')
