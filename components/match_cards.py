...
def render_referee_and_projection_card(db_manager, fixture_data, home_avg, away_avg, league_avg):
    ...
    referee_stats = None
    if referee_name and hasattr(db_manager, "get_referee"):
        try:
            referee_stats = db_manager.get_referee(referee_name)
        except Exception:
            referee_stats = None
    ...
    if referee_stats:
        ...
        st.info(f"👤 **{referee_name}** ({matches_count} pj | **{referee_fouls_avg:.1f}** faltas/p)")
    elif referee_name:
        st.caption(f"👤 **{referee_name}** *(Sin historial)*")
    else:
        st.caption("👤 *Árbitro no designado*")
    ...
    col1.metric("Base", f"{projection['base_projection']}")
    
    ratio = projection['referee_ratio']
    delta_pct = round((ratio - 1.0) * 100, 1)
    col2.metric("Impacto", f"{ratio:.2f}x", delta=f"{delta_pct}%" if ratio != 1.0 else None)
    col3.metric("Ajustada", f"{projection['adjusted_projection']}")
        ...
        for i in range(0, len(matches), 3):
            ...
            for idx, m in enumerate(chunk):
                with cols[idx]:
                    ...
                    # Cálculo de Ventaja vs Promedio de Liga para la alerta del Auto-Simulador
                    home_edge = (home_fouler['fouls_per_90'] - league_avg) if home_fouler else 0.0
                    away_edge = (away_fouler['fouls_per_90'] - league_avg) if away_fouler else 0.0

                    home_badge = " ⚡" if home_edge >= min_edge else ""
                    away_badge = " ⚡" if away_edge >= min_edge else ""
                    ...
                    with st.container(border=True):
                        ...
                        col_h2.caption(
                            f"🔥 {home_fouler['player']}{home_badge} ({home_fouler['fouls_per_90']:.2f}/90m)" 
                            if home_fouler else "—"
                        )
                        ...
                        col_a2.caption(
                            f"🔥 {away_fouler['player']}{away_badge} ({away_fouler['fouls_per_90']:.2f}/90m)" 
                            if away_fouler else "—"
                        )
